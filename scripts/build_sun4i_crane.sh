...
LICHEE_MOD_DIR=${LICHEE_KDIR}/output/vendor/modules/
...
build_kernel()
{
	if [ ! -e .config ]; then
		...
		cp arch/arm/configs/zeki-oem_defconfig .config
	fi

	build_standby
	make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} -j3 uImage modules
